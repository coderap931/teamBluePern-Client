...
| *12/17* | Nearly ready to deploy large UI update | Redesigned the home page. Updated the custom game form heavily and incorporated it with the update feature as well. This ended up being the reason why were not able to deploy yet as we need to update server side acceptance for the forms now due to expanded options. May need to clear database. The landing page is updated with a graphic I designed to direct the user to the sidebar to open the GameChest instead of a blank landing page. Animated, gradient background is default background for all packages unless component writes over the background. Even further login/register validation added. Again, none currently added to live build until debugged with server side. 
